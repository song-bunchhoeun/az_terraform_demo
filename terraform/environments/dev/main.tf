...
module "compute" {
  ...
  app_service_plans = {
    ...
    function = {
      name     = "ASP-rg${local.prefix}${local.environment}sea002-${random_string.asp_suffix.result}"
      os_type  = "Windows"
      sku_name = "Y1"
    }
  }
...
resource "azurerm_cdn_frontdoor_profile" "main" {
  name                = "afd-${local.prefix}-${local.environment}-global-002"
  resource_group_name = module.resource_group.name
