...
module "resource_group" {
  ...
  name     = "rg-${local.prefix}-${local.environment}-sea-001"
...
module "database" {
  ...
  server_name         = "sql-${local.prefix}-${local.env_short}-sea-001"
  resource_group_name = module.resource_group.name
  location            = module.resource_group.location
  server_version      = "12.0"
...
module "compute" {
  ...
  app_service_plans = {
    portal = {
      name     = "plan-${local.prefix}-${local.environment}-sea-001"
      os_type  = "Windows"
      sku_name = "B1"
    }
    api = {
  ...
  linux_web_apps = {
    api_dev = {
      name             = "app-${local.prefix}-api-${local.env_short}-sea-001"
  ...
  windows_web_apps = {
    portal_dev = {
      name             = "app-${local.prefix}-portal-${local.env_short}-sea-001"
  ...
  function_apps = {
    main = {
      name                       = "func-${local.prefix}-${local.env_short}-sea-001"
...
resource "azurerm_api_management" "main" {
  name                = "apim-${local.prefix}-${local.environment}-sea-001"
  location            = module.resource_group.location
  resource_group_name = module.resource_group.name
...
resource "azurerm_api_management_backend" "api_dev" {
  name                = "WebApp_app-${local.prefix}-api-${local.env_short}-sea-001"
  resource_group_name = module.resource_group.name
  api_management_name = azurerm_api_management.main.name
  protocol            = "http"
  url                 = "https://${module.compute.linux_web_app_hostnames["api_dev"]}"
  description         = "app-${local.prefix}-api-${local.env_short}-sea-001"
